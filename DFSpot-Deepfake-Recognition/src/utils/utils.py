...
def load_face_extractor(blazeface_dir, device, fpv):
    ...
    facedet.load_weights(r"D:\DFSpot-Deepfake-Recognition\src\blazeface\blazeface.pth")
    facedet.load_anchors(r"D:\DFSpot-Deepfake-Recognition\src\blazeface\anchors.npy")
...
def extract_predict_annotate(output_dir, ensemble_models, video_glob, video_idxs, transformer, blazeface_dir, device, model_list):
    ...
    facedet = BlazeFace().to(device)
    facedet.load_weights("/content/DFSpot-Deepfake-Recognition/src/blazeface/blazeface.pth")
    facedet.load_anchors("/content/DFSpot-Deepfake-Recognition/src/blazeface/anchors.npy")
    videoreader = VideoReader(verbose=False)

    fpv = fpv_list(video_glob)

    print("FPV of the videos that are to be processed:",[fpv[x] for x in video_idxs if x < len(fpv)])
    ...
    for ne in tqdm(video_idxs, desc='Annotating videos'):
        ...
        while success:
            ...
            h, w, c = img.shape
            cv2.rectangle(img, (int(xmin), int(ymin)), (int(xmax), int(ymax)), rgb, 2)
